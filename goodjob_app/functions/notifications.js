 ...
 */
const notifyTrabajoCancelado = onDocumentUpdated({
  ...
  try {
    ...
    if (tokens.length === 0) {
      console.log(
          `No hay tokens FCM para las postulaciones del trabajo ${trabajoId}`,
      );
      return null;
    }
    ...
    const messages = tokens.map((token) => ({
      ...
      notification: {
        ...
        body:
            `El trabajo "${titulo}" ha sido cancelado, disculpa las molestias.`,
      },
      data: {
        trabajoId,
    ...
    response.responses.forEach((resp, idx) => {
      if (!resp.success) {
        console.log(
            `Token inválido: ${tokens[idx]}, error:`,
            resp.error,
        );
