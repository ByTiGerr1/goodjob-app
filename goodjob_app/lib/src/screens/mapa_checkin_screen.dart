...
class _MapaCheckinScreenState extends State<MapaCheckinScreen> {
 ...
 // Establece un radio de 50 metros (personalizable)
  ...
  if (lat is num && lng is num) {
   ...
   WidgetsBinding.instance.addPostFrameCallback((_) {
    ScaffoldMessenger.of(context).showSnackBar(
     const SnackBar(content: Text('No se encontraron coordenadas válidas para el trabajo. Usando ubicación predeterminada.')),
    );
   });
  }
 }
 ...
 Future<void> _getCurrentLocationAndStartListening() async {
  ...
  if (permission == LocationPermission.denied || permission == LocationPermission.deniedForever) {
   if (mounted) {
    ScaffoldMessenger.of(context).showSnackBar(
     const SnackBar(content: Text('Permiso de ubicación denegado. No se puede realizar el check-in.')),
    );
   }
   return;
  }
 ...
 void _performCheckIn() {
  ...
  if (!_testingMode && !_isInsideGeofence()) {
   ScaffoldMessenger.of(context).showSnackBar(
    const SnackBar(
     content: Text('Debes estar dentro del círculo delimitado para hacer check-in.', style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  ScaffoldMessenger.of(context).showSnackBar(
   const SnackBar(
    content: Text('¡Check-in realizado! Iniciando trabajo...', style: TextStyle(fontWeight: FontWeight.bold)),
 ...
 Widget build(BuildContext context) {
  final isInside = _isInsideGeofence();
    // El botón siempre es verde si estamos en modo de prueba
  final buttonActive = isInside || _testingMode; 
       ...
       MarkerLayer(
        markers: [
         Marker(
          ...
          height: 40,
          point: _trabajoCoords,
          child: const Icon(Icons.work, color: _PRIMARY_COLOR, size: 40),
         ),
        ],
       ),
        ...
        Container(
         ...
         decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
                    // Mensaje de estado: Muestra "Modo Prueba" si está activo
          border: Border.all(color: buttonActive ? Colors.green.shade300 : Colors.red.shade300),
         ),
         child: Text(
          _testingMode 
                        ? '⚠️ MODO PRUEBA: Check-in habilitado' 
                        : isInside 
            ? '¡Estás dentro de la zona de check-in!' 
            : 'Múevete dentro del círculo para hacer check-in.',
          textAlign: TextAlign.center,
          style: TextStyle(
           fontWeight: FontWeight.bold,
           color: _testingMode ? Colors.orange.shade800 : (isInside ? Colors.green.shade800 : Colors.red.shade800),
          ),
         ),
        ),
